...
class FeedForward_cross(nn.Module):
    ...
    #     # x1, x2 = self.dwconv(x).chunk(2, dim=1)
    #     x1, y2 = self.dwconv(x).chunk(2, dim=1)
    #     x = F.gelu(x1) * y2
    #     x = self.project_out(x)
    #     return x
    def forward(self, x, y):
...
class Attention_cross(nn.Module):
    ...
    def forward(self, x, y):
        ...
        qxkxvx = self.qkv_dwconv(self.qkv(x))
        qykyvy = self.qkv_dwconv(self.qkv(y))

        qx, kx, vx = qxkxvx.chunk(3, dim=1)
        qy, ky, vy = qykyvy.chunk(3, dim=1)
        ...
        attn = (qx @ ky.transpose(-2, -1)) * self.temperature
        attn = attn.softmax(dim=-1)
...
class TransformerBlock_dky(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type):
        super(TransformerBlock_dky, self).__init__()

        self.norm1 = LayerNorm(dim, LayerNorm_type)
        self.attn = Attention_cross(dim, num_heads, bias)
        self.norm2 = LayerNorm(dim, LayerNorm_type)
        self.ffn = FeedForward_cross(dim, ffn_expansion_factor, bias)
        # self.ffn = FeedForward(dim, ffn_expansion_factor, bias)

    def forward(self, x, y):
        x = x + self.attn(self.norm1(x), self.norm1(y))
        x = x + self.ffn(self.norm2(x), self.norm2(y))
        # x = x + self.ffn(self.norm2(x))
...
class OverlapPatchEmbed(nn.Module):
    def __init__(self, in_c=64, embed_dim=48, bias=False):
...
class decoNet_restormer(nn.Module):
    def __init__(self,
                 inp_channels=64,
                 out_channels=3,
                 # dim=48,
        ...
        super(decoNet_restormer, self).__init__()

        self.patch_embed = OverlapPatchEmbed(inp_channels, dim)

        self.encoder_level1 = nn.Sequential(*[
            TransformerBlock_dky(dim=dim, num_heads=heads[0], ffn_expansion_factor=ffn_expansion_factor, bias=bias,
                             LayerNorm_type=LayerNorm_type) for i in range(num_blocks[0])])

        self.Trans = TransformerBlock_dky(dim=dim, num_heads=heads[0], ffn_expansion_factor=ffn_expansion_factor, bias=bias,
