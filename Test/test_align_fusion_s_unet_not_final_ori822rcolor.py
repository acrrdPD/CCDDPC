...
def hyper_args():
    ...
    # parser.add_argument('--ir', default='../dataset/Test/512MSRS/ir_warp', type=pathlib.Path)  # MSRS
    # od = 1 + 16 + 16 + 2
    # parser.add_argument('--vis', default='../dataset/Test/512MSRS/vi', type=pathlib.Path)
    parser.add_argument('--ir', default='../dataset/Test/512MSRS/ir_warp', type=pathlib.Path)  # M3FD
    od = 1 + 16 + 16 + 2
    parser.add_argument('--vis', default='../dataset/Test/512MSRS/vi', type=pathlib.Path)
    ...
    parser.add_argument('--dst', default='../results_align_student/240914rml_c1not-varychange-fus-2field-feat-UNETnots-MSRSsame512-1000/SuperF', help='fuse image save folder', type=pathlib.Path)
...
def test(ENet, siNet, AffNet, clip_model, image_trans, warp, FuseNet, FuseNet_t, test_data_loader, dst, device):
    ...
    # for (ir, vis, ir_warp, vis_warp, disp), (ir_path, vis_path) in tqdm_loader:
        ...
        with torch.no_grad():
            ...
            fuse_out = fuse_out_reg_img[:, :1, :, :]
            fused_img = YCbCr2RGB(fuse_out, vi_Cb, vi_Cr)
        ...
        imsave(fused_img, dst / 'fused_img' / file_name)
